...
| ⚔️ **PR merge conflicts** | Tracks every active PR ADO cannot merge (since when, which files), tells the PR and the team **once**, and lists it first on the delivery report — a conflicted PR is never "ready to merge". Resolves on request (`/resolve`, or the Conflicts page) or automatically on its own PRs: merges the target **into** the branch (no rebase, no force‑push), lets the agent settle only the conflicted hunks, and pushes only if no marker is left, no other file was touched, and tests + the security gate pass — otherwise it aborts and the branch is untouched. |
